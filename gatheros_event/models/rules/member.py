...
def rule_2_organizacao_interna_apenas_1_membro(entity, adding=True):
    """
    Organizações internas, por ser de uma pessoa apenas, não podem possuir mais
    de 1 membro.
    """
    if adding is True \
...
def rule_3_organizacao_interna_unico_membro_admin(entity, adding=True):
    """
    Organizações internas, por ser de uma pessoa apenas, seu único membro deve
    ser ADMIN
    """
...
def rule_4_nao_remover_member_organizacao_interna(entity):
    ...
    if entity.organization.internal is True:
        raise IntegrityError(
            'Impossível remover membro. Organização interna deve possuir um'
            ' membro ADMIN principal.'
        )
