...
(function ($, AjaxSender) {
    ...
    cgsy.FormHelper = function (form_el) {
        form_el = $(form_el);

        var url = form_el.attr('action');
        var method = form_el.attr('method');

        this.sender = new AjaxSender(url);

        this.send = function () {
            return this.sender.send(method, new FormData(form_el[0]));
        };
    };

})(jQuery, window.cgsy.AjaxSender);
