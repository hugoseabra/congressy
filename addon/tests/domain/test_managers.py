...
class ServiceManagerPersistenceTest(ManagerPersistenceTestCase):
    ...
    def setUp(self):
        ...
        self.data = {
            ...
            'name': 'optional name',
            'schedule_start_0': schedule_start.strftime('%d/%m/%Y'),
            'schedule_start_1': schedule_start.strftime('%H:%M'),
            'schedule_end_0': schedule_end.strftime('%d/%m/%Y'),
            'schedule_end_1': schedule_end.strftime('%H:%M'),
            'date_end_sub_0': schedule_end.strftime('%d/%m/%Y'),
            'date_end_sub_1': schedule_end.strftime('%H:%M'),
            'published': True,
            'created_by': 'test user',
            'modified_by': 'test user',
            'price': format(decimal.Decimal(42.42), '.2f'),
