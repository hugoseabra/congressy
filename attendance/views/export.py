...
def export_attendance(attendance_service_pk=None):
    ...
    ws2 = wb.create_sheet(title='Detalhamento')

    subscriptions = list()
    for sub in Subscription.objects.filter(event__attendance_services=service):
        subscriptions.append(sub)

    if len(subscriptions) == 0:
        ws2.append(["Nenhuma inscrição neste serviço de atendimento"])
    else:
        export_attendance_detailed_subscriptions(ws2, subscriptions, service)
...
def export_attendance_detailed_subscriptions(worksheet, subscriptions,
                                             attendance):
    """ Exporta dados de inscrição. """
    ...
    for sub in subscriptions:
        ...
        checkins = [c for c in checkins]

        checkin_pks = [c.pk for c in checkins]

        checkouts = Checkout.objects.filter(
            checkin__in=checkin_pks
        )

        checkouts = [c for c in checkouts]
        ...
        for i in range(1, len(checkins)):
            checkin_str += "Checkin {}º feito ás {} \n".format(
                i,
                checkins[i - 1].created_on.strftime('%H:%M:%S de %d/%m/%Y '))
        ...
        checkout_str = ''
        for i in range(1, len(checkouts)):
            checkout_str += "Checkout {}º feito ás {} \n".format(
                i,
                checkouts[i - 1].created_on.strftime('%H:%M:%S de %d/%m/%Y '))

        if checkout_str == '':
            checkout_str = '----'

        collector[row_idx].append(checkout_str)
